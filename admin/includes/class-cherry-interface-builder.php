...
class Cherry_Interface_Builder {
	...
	public function add_form_item( $args = array() ) {
		$default = array(
			...
			'options'				=> '',
			'placeholder'			=> '',
			'upload_button_text'	=> __( 'Choose Media', 'non-cherry' ),
			'remove_button_text'	=> __( 'Remove Media', 'non-cherry' ),
			'return_data_type'		=> 'id',
			'multi_upload'			=> true,
			'display_image'			=> true,
			'display_input'			=> true,
			...
			'hint'					=> '',
			'toggle'				=> array(
				'true_toggle'		=> __( 'On', 'non-cherry' ),
				'false_toggle'		=> __( 'Off', 'non-cherry' ),
				'true_slave'		=> '',
				'false_slave'		=> ''
			),
			'master'				=> '',
	...
	private function wrap_item( $item, $id, $item_id, $class, $title, $label, $description, $master, $hint ) {
		...
		$export_check = new UI_Switcher(
			array(
				...
				'style'	=> 'small',
				'toggle'		=> array(
					'true_toggle'	=> __( 'Yes', 'non-cherry' ),
					'false_toggle'	=> __( 'No', 'non-cherry' ),
				),
			)
		);
